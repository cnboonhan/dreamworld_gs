...
time. **RMF** simulates the building's infrastructure from an annotated
floorplan: doors, lifts, the nav graph. **Gaussian splatting** shows what
each place actually looks like, generated from a single panorama taken
there. **A video model** fills the seconds reality hides between two
photographs: the door swinging open as you cross an edge.

Everything runs on this box, offline, behind one forwarded port.

## The surfaces

```
just up                                  build and start everything
ssh -L 8080:localhost:8080 <box>         one tunnel carries it all
```

| where | what |
| --- | --- |
| `/` | the landing page |
| `/sim_editor` | traffic editor over noVNC — author the building's STRUCTURE: walls, doors, lifts, the scale measurement |
